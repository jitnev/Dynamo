...
namespace Dynamo.Engine
{
    ...
    public class LibraryServices : LogSourceBase, IDisposable
    {
        ...
        internal List<string> GetNodesFromZeroTouchAssem(string library)
        {

            if (null == library)
                throw new ArgumentNullException();

            List<string> assemblyNodes = new List<string>();
            ...
            try
            {
                ...
                foreach (var classNode in loadedClasses)
                {
                    foreach (ProcedureNode proc in classNode.ProcTable.Procedures)
                    {
                        ...
                        assemblyNodes.Add(className[className.Length-1] + "." +  proc.Name);
                    }
                }
